...
class VideoController {
  ...
  Future<PlayerResponse> getPlayerResponse(VideoId videoId, YoutubeApiClient client, {WatchPage? watchPage}) async {
    ...
    if (body['context']!['client']['clientName'] == 'IOS') {
      body['context']!['client']!['visitorData'] = await _extractVisitorData(httpClient, client);
    }
  ...
  Future<String> _extractVisitorData(YoutubeHttpClient http, YoutubeApiClient client) async {
    if (_visitorData != null) {
      return _visitorData!;
    }

    var response = await http.getString('https://www.youtube.com/sw.js_data', headers: {
